...
>
    ...
    <div class="flex items-center gap-3 px-5 py-5 border-b border-white/10">
        ...
        <div>
            ...
            <div class="text-xs text-gray-400 leading-tight">Administrator Area</div>
        </div>
    </div>
    ...
    <div class="px-5 py-3 border-b border-white/10">
        <div class="bg-white/5 rounded-xl px-3 py-2.5">
            ...
            <div class="text-sm font-semibold text-white truncate">{{ auth()->user()->school->name ?? 'SMK Negeri Digital' }}</div>
        </div>
    </div>
    ...
    <nav class="flex-1 overflow-y-auto px-3 py-4 space-y-0.5">
        ...
        <a href="{{ route('admin.dashboard') }}"
           class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all
            ...
            <i class="fas fa-th-large w-4 text-center {{ request()->routeIs('admin.dashboard') ? 'text-evergreen-400' : 'text-gray-500' }}"></i>
            Dashboard
        </a>
        ...
        <a href="{{ route('admin.books.index') }}"
           class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all
            ...
            <i class="fas fa-book w-4 text-center {{ request()->routeIs('admin.books.*') ? 'text-evergreen-400' : 'text-gray-500' }}"></i>
            Katalog Buku
        </a>
        ...
        <a href="{{ route('admin.transactions.index') }}"
           class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all
            ...
            <i class="fas fa-exchange-alt w-4 text-center {{ request()->routeIs('admin.transactions.*') ? 'text-evergreen-400' : 'text-gray-500' }}"></i>
            Transaksi
        </a>
        ...
        <a href="{{ route('admin.members.index') }}"
           class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all group
            ...
            <i class="fas fa-users w-4 text-center {{ request()->routeIs('admin.members.*') ? 'text-evergreen-400' : 'text-gray-500' }}"></i>
            Anggota
            {{-- Pending badge --}}
            @php $pendingCount = \App\Models\User::where('school_id', auth()->user()->school_id)->where('status','pending')->count(); @endphp
            @if($pendingCount > 0)
        ...
        <a href="{{ route('admin.classes.index') }}"
           class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all
            ...
            <i class="fas fa-chalkboard w-4 text-center {{ request()->routeIs('admin.classes.*') ? 'text-evergreen-400' : 'text-gray-500' }}"></i>
            Kelas & Jurusan
        </a>
        ...
        <a href="{{ route('admin.settings') }}"
           class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all
            ...
            <i class="fas fa-cog w-4 text-center {{ request()->routeIs('admin.settings') ? 'text-evergreen-400' : 'text-gray-500' }}"></i>
            Pengaturan
        </a>
    </nav>
